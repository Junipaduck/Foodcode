...
//    	List<ReviewVO> reviewList = reviewService.reviewList(review);
    	...
    	List<ReviewVO> reviewList = reviewService.getOwnerReivewList(listLimit, startRow);
    	
    	// 1. 전체 게시물 수 조회 
    	int listCount = reviewService.getOwnerReviewListCount();
    	
    	// 2. 한 페이지에 표시할 페이지 목록 갯수 설정
    	int pageListLimit = 5;
